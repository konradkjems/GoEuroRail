...
interface CityDataWithPopulation extends CityData {
  population: number;
}

const cities: CityDataWithPopulation[] = [];

createReadStream('../../cities /geonames-all-cities-with-a-population-1000@public.csv')
  .pipe(csv({ 
    separator: ';',
    headers: [
  ...
  .on('data', (row: any) => {
    ...
    if (europeanCountryCodes.has(row.country_code)) {
      ...
      const [lat, lng] = row.coordinates.split(',').map((coord: string) => parseFloat(coord.trim()));
  ...
  .on('end', () => {
    ...
    const topCities: CityData[] = cities.slice(0, 1000).map(({ population, ...rest }) => rest);
